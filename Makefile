...
package: build ## Package fonts and create GitHub release for homebrew
	@VERSION=$$(grep "Version:" premade-configs/config.kg.yaml | sed 's/.*"\([^"]*\)".*/\1/') && \
	...
	cd fonts && zip -r -X ../recursive-$$VERSION.zip RecursiveKG/ && cd .. && \
	echo "🔐 Calculating SHA256..." && \
	SHA=$$(shasum -a 256 recursive-$$VERSION.zip | awk '{print $$1}') && \
	echo "$$SHA" | pbcopy && \
	...
	if [ ! -d "../homebrew-tools" ]; then \
		...
		cd .. && git clone https://github.com/kaushikgopal/homebrew-tools.git && cd recursive-code-config; \
	fi && \
	cd ../homebrew-tools && (git pull origin main || git pull origin master) && cd ../recursive-code-config && \
	...
	git commit -m "update: recursive kg $$VERSION" && \
	(git push origin main || git push origin master) && \
